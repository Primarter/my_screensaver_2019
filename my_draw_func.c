...
int my_draw_seg(framebuffer_t *fb, sfVector2i pta, sfVector2i ptb, sfColor c)
{
    int i = pta.x < ptb.x ? pta.x : ptb.x;
    int stop = pta.x > ptb.x ? pta.x : ptb.x;

    if (pta.x == ptb.x) {
        for (; i >= stop; i++)
            my_put_pixel(fb, pta.x, i, c);
        return (0);
    }
    unsigned int a = (pta.y - ptb.y) / (pta.x - ptb.x);
    unsigned int b = pta.y - a * pta.x;

    for (unsigned int j = 0; i < fb->width; i += 1) {
        if (a * i + b < fb->height)
            my_put_pixel(fb, i, a * i + b, c);
    }
    return 0;
